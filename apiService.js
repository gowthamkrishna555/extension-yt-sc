...
window.ApiService = (function () {
  ...
  async function analyzeTone(input) {
    try {
      const serverUrl = `${API_BASE_URL}/analyze-tone`;
      const response = await fetch(serverUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      console.error("Tone analysis request failed:", err);
      return null;
    }
  }

  // Add to ApiService.js
  async function getReadabilityScore(input) {
    try {
      const serverUrl = `${API_BASE_URL}/readability`;
      const response = await fetch(serverUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      console.error("Readability score request failed:", err);
      return null;
    }
  }

  // Add to ApiService.js
  async function checkPlagiarism(input) {
    try {
      if (!input || !input.trim()) {
        return null;
      }

      const serverUrl = `${API_BASE_URL}/plagiarism`;
  ...
  // Add to ApiService.js
  async function getSynonymSuggestions(input) {
    try {
      const serverUrl = `${API_BASE_URL}/synonyms`;
      ...
      console.error("Synonym suggestions request failed:", err);
      return null;
    }
  }
  ...
  async function detectLanguage(input) {
    try {
      const serverUrl = `${API_BASE_URL}/detect-language`;
  ...
  async function translateText(input, targetLang) {
    try {
      const serverUrl = `${API_BASE_URL}/translate`;
  ...
  async function fetchEnhancedAnalysis(input) {
    try {
      ...
      console.error("Enhanced analysis request failed:", err);
      return null;
    }
  }
